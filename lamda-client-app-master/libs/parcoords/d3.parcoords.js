...
var events = d3.dispatch.apply(this,["render", "resize", "highlight", "brush", "brushend", "axesreorder"].concat(d3.keys(__))),
    ...
    axisCateg = d3.svg.axis().orient("left").ticks(5),
...
var side_effects = d3.dispatch.apply(this,d3.keys(__))
  ...
  .on("data", function(d,i) {
    if (flags.shadows){paths(__.data, ctx.shadows,i);}
  })
...
pc.render.default = function() {
  ...
  __.data.forEach((d,i)=>{path_foreground(d,i,__.data.length,{});});
};
...
pc.renderBrushed.default = function() {
  ...
  if (isBrushed()) {
    __.brushed.forEach((d,i)=>{path_brushed(d,i,__.brushed.length,{})});
  }
};
...
d3.rebind(pc, axisCateg , "ticks", "orient", "tickValues", "tickSubdivide", "tickSize", "tickPadding", "tickFormat");
...
function flipAxisAndUpdatePCP(dimension) {
  ...
  if(__.types[dimension]==="number") {
      ...
      d3.select(this.parentElement)
          .transition()
          .duration(1100)
          .call(axisCateg.scale(yscale[dimension]));
          // .call(axis.scale(yscale[dimension]));
  }
  pc.render();
  if (flags.shadows) paths(__.data, ctx.shadows);
...
function callAxis(selectedElement,dimension){
    if(__.types[dimension]==="number") {
        ...
        const tickSize=axisCateg.tickSize(); // default should be 6;
        ...
        selectedElement.call(axisCateg.scale(yscale[dimension]).tickValues(tickValues))
            .selectAll('.tick text')
...
pc.highlight = function(data) {
  ...
  data.forEach((d,i)=>{path_highlight(d,i,data.length,{});});
...
d3.renderQueue = (function(func) {
  ...
  rq.render = function() {
    ...
    rq.invalidate = function() {
        valid = false;
        if(rq._renderingMonitoring!==null)rq._renderingMonitoring.stopRendering(rq._action);
    };
    if(rq._queue.length===0)return;
    if(rq._renderingMonitoring!==null)rq._renderingMonitoring.startRendering(rq._action,rq._queue.length);
    function doFrame() { // !!! set withApply to true inside doFrame() because called in d3.timer (outside angular scope)
      if (!valid) {
          if(rq._renderingMonitoring!==null)rq._renderingMonitoring.stopRendering(rq._action,true);
          return true;
      }
      ...
      if(rq._renderingMonitoring!==null)rq._renderingMonitoring.setRemainingRenderingNb(rq._action,remaining,true);
      if(remaining&&rq._renderingMonitoring!==null)rq._renderingMonitoring.stopRendering(rq._action,true);
  ...
  rq.data = function(data) {
    ...
    rq._queue = data.slice(0);
      ...
      rq._queue.sort((a,b)=>{
          const colorA=d3.functor(rq._color)(a, i); // get color from data value a;
          const colorB=d3.functor(rq._color)(b, i); // get color from data value b;
          if ( colorA < colorB ){
              return 1;
      ...
      const colorCheck=rq._queue.map((d, i)=>{
          return d3.functor(rq._color)(d, i)// get color from data value d; to check consistency
      });
      ...
      rq._queue.forEach((d,i)=>{
          if(i===0){
              ...
              const colorA=d3.functor(rq._color)(rq._queue[i-1], i); // get color from rq._queue[i-1]
              const colorB=d3.functor(rq._color)(d, i); // get color from d
              if(colorA!==colorB){
                  rq._forceSetStyle[i]=true;
              }
          }
