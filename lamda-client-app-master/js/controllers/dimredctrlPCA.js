...
angular.module('LAMDAClientApp.controllers')
  .controller('DimRedCtrlPCA', function ($scope, $modal, Analytics, R, DimRedPlot, DataSet,AppStatus) {
    ...
    $scope.openSelectionDialog = function (drMethodName) {
      ...
      var dialog = $modal.open({
        ...
        resolve: {
          variables: function () {
            return $scope.variables.filter(function(d){return R.is.numeric(d.type)});
          },
          selected: function () {
            return $scope.selectedVariables.filter(function(d){return R.is.numeric(d.type)});
          }
        }
      });
    ...
    $scope.reduceDimensionality = function () {
      ...
      $scope.configurationInvalid = true;
      AppStatus.setStatus("Calling dimensionality reduction (PCA)...")
