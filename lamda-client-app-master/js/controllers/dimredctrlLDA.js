...
angular.module('LAMDAClientApp.controllers')
  .controller('DimRedCtrlLDA', function ($scope, $modal, Analytics, R, DimRedPlot, DataSet, AppStatus) {
    ...
    $scope.$on('Analytics::dimRedMethodsAvailable', function (e, methods) {
      // this version of the controller only supports PCA
      $scope.dimRedMethods = _.filter(methods, function (m) { return m.name === "lda"; });
    ...
    $scope.reduceDimensionality = function () {
      ...
      $scope.configurationInvalid = true;
      AppStatus.setStatus("Calling dimensionality reduction (LDA)...")
