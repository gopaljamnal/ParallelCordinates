...
angular.module('LAMDAClientApp.controllers')
  .controller('ChartsCtrl', function ($scope, DataSet) {

    'use strict';

    $scope.location = false;

    $scope.dataTableIsActive = true;
    $scope.dimRedIsActive=false;
    $scope.activateDimredTab=function(){
        $scope.dimRedIsActive=true;
        $scope.dataTableIsActive=false;
    };
    $scope.$on('Analytics::DimensionalityReductionCalled',function (e, parameters) {
        $scope.activateDimredTab();
    });
