...
angular.module('LAMDAClientApp.controllers')
  .controller('ClusterConfigCtrl', function ($scope, $modalInstance, options, DataSet) {
    ...
    $scope.specified = {
      ...
      identifier:  ($scope.options.methods[0]?$scope.options.methods[0].name:"NoClusteringMethodsFound") + '_' + previousCentersCount + '_0',
    ...
    $scope.hideCenters = function () {
      if ($scope.specified.method.name !== "kmeans" &&
        $scope.specified.method.name !== "kmeansNN" ) {
